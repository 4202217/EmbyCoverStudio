...
- 封面墙以真实海报为视觉主角，零装饰、零元数据，封面本身是唯一内容
...
### Navigation
- **Style:** 侧边栏 224px，固定高度并钉在视口内（`sticky top-0 h-[100dvh]`），底部 System 状态卡与版本号永久可见；半透明卡片底 + 背景模糊；导航项 8px 圆角，活跃态为 `primary/10` 底 + 青色文字 + 3px 左侧竖条。
- **Live Progress:** 批量封面任务进行中时，System 卡上方显示实时进度卡（进度条 + 完成数 + 当前目标），侧边栏在任务期间以 2 秒轮询刷新，空闲恢复 15 秒。
- **Mobile treatment:** `lg` 以下切换为顶部粘性栏 + 横向滚动的胶囊导航，活跃胶囊 `primary/10` 底 + 青色文字。

### Poster Cards（标志性组件）
- **Corner Style:** 10px；比例 16:9（媒体库）或 9:16（合集）。
- **Behavior:** 成果墙是纯展示——封面下方不出现名称、时间、触发方式等任何元数据（名称已印在封面图内）；hover 时轻微上浮（`-translate-y-0.5`）+ 缩放 1.02 + 阴影升 `pop`，点击打开预览弹窗查看详情。
...
- **Do** 让封面墙保持纯净：封面本身是唯一内容，任何元数据只进预览弹窗。
...
- **Don't** 给封面叠加彩色光晕或装饰性标签；封面墙没有 hover 文字层。
