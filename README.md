...
- **选图依据**：最新入库 / 最新发行 / 随机 / 手动选择（仅单图海报支持手动选择）
...
「设置 → 配置与数据备份」支持导出/导入 JSON 备份，以及 WebDAV 自动/手动同步（可勾选同步内容：设置、媒体库/合集配置、任务记录）。封面图片不包含在备份内，导入后重新生成即可。
